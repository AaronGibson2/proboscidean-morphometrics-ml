...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--title", help="Figure title; defaults to the run name")
...
def class_color(name: str) -> str:
    ...
    if "love" in normalized:
        return "#d62728"
    if "mixson" in normalized:
        return "#ff7f0e"
    if "tyner" in normalized:
        return "#4c78a8"
    return "#777777"
    ...
    if result is not None:
        ...
        for axis, x_name, y_name, title in (
            ...
            for class_name, group in coordinates.groupby("class_str"):
                axis.scatter(
                    group[x_name], group[y_name], s=55, alpha=0.85,
                    color=class_color(class_name), label=class_name.replace("_", " "),
                )
            validation = coordinates[coordinates["dataset"] == "val"]
            axis.scatter(
                validation[x_name], validation[y_name], s=120, facecolors="none",
        ...
        figure.suptitle(args.title or args.run_name.replace("_", " "))
